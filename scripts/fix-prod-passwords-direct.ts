...
async function recreateUsersWithAPI() {
  ...
  execSync(`psql "${DATABASE_URL}" -c "DELETE FROM account; DELETE FROM \\\"user\\\";"`, {
    stdio: 'inherit',
  })
...
async function main() {
  try {
    ...
    execSync(
      `psql "${DATABASE_URL}" -c "SELECT u.email, u.role, LENGTH(a.password) as pwd_len FROM \"user\" u JOIN account a ON u.id = a.user_id ORDER BY u.email;"`,
      { stdio: 'inherit' }
    )
