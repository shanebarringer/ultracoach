...
async function analyzeMigration() {
  ...
  try {
    ...
    for (const user of differentIdUsers) {
      ...
      const totalRecords = parseInt(trainingPlansAsCoach.rows[0].count) + 
      ...
      if (totalRecords > 0) {
        ...
        if (trainingPlansAsCoach.rows[0].count > 0) console.log(`     • ${trainingPlansAsCoach.rows[0].count} training plans as coach`);
        if (trainingPlansAsRunner.rows[0].count > 0) console.log(`     • ${trainingPlansAsRunner.rows[0].count} training plans as runner`);
        if (messagesAsSender.rows[0].count > 0) console.log(`     • ${messagesAsSender.rows[0].count} messages as sender`);
        if (messagesAsRecipient.rows[0].count > 0) console.log(`     • ${messagesAsRecipient.rows[0].count} messages as recipient`);
        if (notifications.rows[0].count > 0) console.log(`     • ${notifications.rows[0].count} notifications`);
      }
    }
...
if (require.main === module) {
  analyzeMigration().catch(console.error).finally(() => pool.end());
}
