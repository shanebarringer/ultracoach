...
function cleanupPasswordFiles() {
  ...
  passwordFiles.forEach(filename => {
    ...
    if (fs.existsSync(filepath)) {
      ...
      cleanedCount++
      logger.info(`🗑️ Removed password file: ${filename}`)
    }
  })
    ...
    if (fs.existsSync(dirPath)) {
      passwordFiles.forEach(filename => {
        ...
        if (fs.existsSync(filepath)) {
          fs.unlinkSync(filepath)
          cleanedCount++
          logger.info(`🗑️ Removed password file: ${dir}/${filename}`)
        }
      })
    }
  })
  ...
  if (cleanedCount > 0) {
    logger.info(`✅ Cleaned up ${cleanedCount} password files`)
  } else {
    logger.info('ℹ️ No password files found to clean up')
  }
}
