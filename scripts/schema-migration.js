...
async function schemaMigration() {
  ...
  try {
    ...
    const constraints = [
      { table: 'training_plans', column: 'coach_id', constraint: 'training_plans_coach_id_users_id_fk' },
      { table: 'training_plans', column: 'runner_id', constraint: 'training_plans_runner_id_users_id_fk' },
      { table: 'messages', column: 'sender_id', constraint: 'messages_sender_id_users_id_fk' },
      { table: 'messages', column: 'recipient_id', constraint: 'messages_recipient_id_users_id_fk' },
    ...
    for (const { table, column } of columnChanges) {
      ...
      if (typeCheck.rows[0] && typeCheck.rows[0].data_type === 'uuid') {
        ...
        console.log(`  ℹ️  ${table}.${column} is already TEXT (${typeCheck.rows[0]?.data_type || 'unknown'})`);
      }
    }
    ...
    for (const mapping of userMappingsResult.rows) {
      ...
      const userTotal = coachUpdates.rowCount + runnerUpdates.rowCount + 
                       senderUpdates.rowCount + recipientUpdates.rowCount + 
...
if (require.main === module) {
  schemaMigration().catch(console.error).finally(() => pool.end());
}
