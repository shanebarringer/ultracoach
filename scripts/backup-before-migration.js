...
async function backupData() {
  try {
    ...
    const orphanedTrainingPlans = trainingPlans.rows.filter(tp => !tp.coach_email || !tp.runner_email);
    if (orphanedTrainingPlans.length > 0) {
      console.log(`⚠️  Warning: ${orphanedTrainingPlans.length} training plans with missing user references`);
    }
