import { pgTable, unique, pgPolicy, text, boolean, timestamp, foreignKey, index, uuid, integer, varchar, numeric } from "drizzle-orm/pg-core"
...
export const betterAuthUsers = pgTable("better_auth_users", {
	...
	pgPolicy("Users can manage their own data", { as: "permissive", for: "all", to: ["public"], using: sql`(id = current_setting('app.current_user_id'::text, true))` }),
]);
...
export const betterAuthSessions = pgTable("better_auth_sessions", {
	...
	index("idx_better_auth_sessions_expires_at").using("btree", table.expiresAt.asc().nullsLast().op("timestamptz_ops")),
	index("idx_better_auth_sessions_user_id").using("btree", table.userId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [betterAuthUsers.id],
...
export const trainingPhases = pgTable("training_phases", {
	...
	index("idx_training_phases_phase_order").using("btree", table.phaseOrder.asc().nullsLast().op("int4_ops")),
]);
...
export const conversations = pgTable("conversations", {
	...
	index("idx_conversations_coach_id").using("btree", table.coachId.asc().nullsLast().op("text_ops")),
	index("idx_conversations_runner_id").using("btree", table.runnerId.asc().nullsLast().op("text_ops")),
	foreignKey({
	...
	pgPolicy("Users can manage their conversations", { as: "permissive", for: "all", to: ["public"], using: sql`((coach_id = current_setting('app.current_user_id'::text, true)) OR (runner_id = current_setting('app.current_user_id'::text, true)))` }),
]);
...
export const messages = pgTable("messages", {
	...
	pgPolicy("Users can manage their messages", { as: "permissive", for: "all", to: ["public"], using: sql`((sender_id = current_setting('app.current_user_id'::text, true)) OR (recipient_id = current_setting('app.current_user_id'::text, true)))` }),
]);
...
export const trainingPlans = pgTable("training_plans", {
	...
	index("idx_training_plans_coach_id").using("btree", table.coachId.asc().nullsLast().op("text_ops")),
	index("idx_training_plans_runner_id").using("btree", table.runnerId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.coachId],
			foreignColumns: [betterAuthUsers.id],
	...
	pgPolicy("Coaches can manage their training plans", { as: "permissive", for: "all", to: ["public"], using: sql`(coach_id = current_setting('app.current_user_id'::text, true))` }),
	pgPolicy("Runners can view their training plans", { as: "permissive", for: "select", to: ["public"] }),
]);
...
export const notifications = pgTable("notifications", {
	...
	pgPolicy("Users can manage their notifications", { as: "permissive", for: "all", to: ["public"], using: sql`(user_id = current_setting('app.current_user_id'::text, true))` }),
]);
...
export const races = pgTable("races", {
	...
	index("idx_races_distance_type").using("btree", table.distanceType.asc().nullsLast().op("text_ops")),
	foreignKey({
...
export const planTemplates = pgTable("plan_templates", {
	...
	index("idx_plan_templates_difficulty_level").using("btree", table.difficultyLevel.asc().nullsLast().op("text_ops")),
	index("idx_plan_templates_distance_type").using("btree", table.distanceType.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.createdBy],
			foreignColumns: [betterAuthUsers.id],
...
export const workouts = pgTable("workouts", {
	...
	index("idx_workouts_training_plan_id").using("btree", table.trainingPlanId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.trainingPlanId],
			foreignColumns: [trainingPlans.id],
			name: "workouts_training_plan_id_fkey"
		}).onDelete("cascade"),
	pgPolicy("Users can manage workouts through training plans", { as: "permissive", for: "all", to: ["public"], using: sql`(training_plan_id IN ( SELECT training_plans.id
   FROM training_plans
...
export const messageWorkoutLinks = pgTable("message_workout_links", {
	id: uuid().defaultRandom().primaryKey().notNull(),
...
export const templatePhases = pgTable("template_phases", {
	...
	index("idx_template_phases_phase_id").using("btree", table.phaseId.asc().nullsLast().op("uuid_ops")),
	index("idx_template_phases_template_id").using("btree", table.templateId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
