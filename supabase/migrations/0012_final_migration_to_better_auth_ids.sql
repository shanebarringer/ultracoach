-- Final Migration to Better Auth IDs
-- This migration properly handles the UUID to text conversion

BEGIN;

-- Step 1: Show migration status
SELECT 'Starting Better Auth ID migration' as status;

-- Step 2: Disable RLS policies temporarily
...
-- Step 4: Drop all foreign key constraints that reference users table
...
-- Step 5: Create backup tables before migration
...
-- Step 6: Create mapping table with proper type handling
...
-- Step 7: Update users table to use Better Auth IDs as primary key
...
-- Step 8: Update all foreign key references to use Better Auth IDs
...
-- Step 10: Re-enable RLS and recreate policies with text IDs
