-- Final Comprehensive Migration to Better Auth IDs
-- This migration handles ALL tables and policies that reference user columns

BEGIN;

-- Step 1: Show migration status
SELECT 'Starting FINAL comprehensive Better Auth ID migration' as status;

-- Step 2: Get all policies that might reference user columns
SELECT 'Current policies that might block migration:' as info;
SELECT schemaname, tablename, policyname 
FROM pg_policies 
WHERE policyname ILIKE '%coach%' OR policyname ILIKE '%training%' OR policyname ILIKE '%workout%' 
   OR policyname ILIKE '%user%' OR policyname ILIKE '%auth%'
ORDER BY tablename;

-- Step 3: Disable RLS on ALL tables that might have user-related policies
...
-- Step 4: Drop ALL policies that might reference user columns
...
-- Step 6: Create backup tables with timestamp
CREATE TABLE users_backup_20250716 AS SELECT * FROM users;
CREATE TABLE training_plans_backup_20250716 AS SELECT * FROM training_plans;
CREATE TABLE workouts_backup_20250716 AS SELECT * FROM workouts;
CREATE TABLE messages_backup_20250716 AS SELECT * FROM messages;
CREATE TABLE notifications_backup_20250716 AS SELECT * FROM notifications;
CREATE TABLE typing_status_backup_20250716 AS SELECT * FROM typing_status;
CREATE TABLE races_backup_20250716 AS SELECT * FROM races;
CREATE TABLE plan_templates_backup_20250716 AS SELECT * FROM plan_templates;

-- Step 7: Create mapping table
...
SELECT old_id, new_id, email FROM user_id_mapping LIMIT 5;

-- Step 8: Update users table to use Better Auth IDs
...
-- Step 11: Update notifications table
...
-- Step 16: Re-enable RLS on all tables
SELECT 'Re-enabling RLS and recreating policies...' as status;
...
-- Step 17: Recreate all RLS policies with text IDs
-- Users policies
...
BEGIN
    SELECT COUNT(*) INTO user_count_before FROM users_backup_20250716;
...
-- Step 19: Show final status
SELECT 'Migration completed successfully!' as status;
SELECT 'Total users migrated:' as info, COUNT(*) as count FROM users;
SELECT 'Sample user IDs after migration:' as info;
SELECT id, email FROM users LIMIT 5;

COMMIT;

-- Final success message
SELECT 'FINAL COMPREHENSIVE DATABASE MIGRATION TO BETTER AUTH IDs COMPLETED SUCCESSFULLY!' as final_status;