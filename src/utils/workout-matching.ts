...
export const defaultMatchingOptions: MatchingOptions = {
  ...
  durationTolerance: 0.2, // 20% duration tolerance
...
export function batchMatchActivities(
  ...
  logger.info(
    `Starting batch matching for ${activities.length} activities against ${workouts.length} workouts`
  )
...
export function generateMatchingSummary(
  ...
  const byConfidence = allMatches.reduce(
    (acc, match) => {
  ...
  if (byConfidence.probable > 0) {
    suggestions.push(
      `${byConfidence.probable} activities are probable matches - review minor differences`
    )
  }
  if (byConfidence.possible > 0) {
    suggestions.push(
      `${byConfidence.possible} activities are possible matches - check for conflicts`
    )
  }
