...
export const garmin_connections = pgTable(
  'garmin_connections',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    user_id: text('user_id')
  ...
  table => ({
    userIdUnique: unique('garmin_connections_user_id_unique').on(table.user_id),
    garminUserIdUnique: unique('garmin_connections_garmin_user_id_unique').on(table.garmin_user_id),
  })
)
