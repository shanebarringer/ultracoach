...
export const workouts = pgTable('workouts', {
  id: uuid('id').primaryKey().defaultRandom(),
  training_plan_id: uuid('training_plan_id').notNull().references(() => training_plans.id, { onDelete: 'cascade' }),
...
export const better_auth_accounts = pgTable('better_auth_accounts', {
    ...
    userId: text('user_id').notNull().references(() => better_auth_users.id, { onDelete: 'cascade' }),
