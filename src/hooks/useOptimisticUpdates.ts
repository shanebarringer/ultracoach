...
export function useOptimisticUpdates() {
  ...
  const updateWorkoutOptimistic = useCallback(
    async (workoutId: string, updates: Partial<Workout>, apiCall: () => Promise<Workout>) => {
      // Immediately update UI with optimistic state
      const optimisticWorkout = { ...updates, id: workoutId, isPending: true }

      setWorkouts(prev =>
        prev.map(workout =>
          workout.id === workoutId ? { ...workout, ...optimisticWorkout } : workout
        )
      )
      ...
      try {
        ...
        setWorkouts(prev =>
          prev.map(workout =>
            workout.id === workoutId ? { ...updatedWorkout, isPending: false } : workout
          )
        )
  ...
  const updateTrainingPlanOptimistic = useCallback(
    async (
      planId: string,
      ...
      setTrainingPlans(prev =>
        prev.map(plan => (plan.id === planId ? { ...plan, ...optimisticPlan } : plan))
      )
      ...
      try {
        ...
        setTrainingPlans(prev =>
          prev.map(plan => (plan.id === planId ? { ...updatedPlan, isPending: false } : plan))
        )
        ...
        setTrainingPlans(prev =>
          prev.map(plan =>
            plan.id === planId ? { ...plan, ...updates, isPending: false, hasError: true } : plan
          )
        )
  ...
  const sendMessageOptimistic = useCallback(
    async (
      ...
      try {
        ...
        setMessages(prev =>
          prev.map(msg =>
            msg.id === optimisticMessage.id ? { ...sentMessage, isPending: false } : msg
          )
        )
  ...
  const pendingStates = useMemo(
    () => ({
      ...
      hasPendingPlans: trainingPlans.some(
        (p: TrainingPlan & { isPending?: boolean }) => p.isPending
      ),
      hasPendingMessages: messages.some(
        (m: MessageWithUser & { isPending?: boolean }) => m.isPending
      ),
      pendingWorkoutIds: workouts
        .filter((w: Workout & { isPending?: boolean }) => w.isPending)
        .map(w => w.id),
      pendingPlanIds: trainingPlans
        .filter((p: TrainingPlan & { isPending?: boolean }) => p.isPending)
        .map(p => p.id),
      pendingMessageIds: messages
        .filter((m: MessageWithUser & { isPending?: boolean }) => m.isPending)
        .map(m => m.id),
    }),
