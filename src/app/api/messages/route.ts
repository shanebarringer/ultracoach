...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: rawMessages, error: messagesError } = await supabaseAdmin
      ...
      .select('*')
      .or(
        `and(sender_id.eq.${session.user.id},recipient_id.eq.${recipientId}),and(sender_id.eq.${recipientId},recipient_id.eq.${session.user.id})`
      )
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (workoutId) {
      ...
      const hasAccess =
        trainingPlan.coach_id === session.user.id || trainingPlan.runner_id === session.user.id
      if (!hasAccess) {
