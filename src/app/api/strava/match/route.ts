...
import { batchMatchActivities, matchActivityToWorkouts, type WorkoutMatch, type MatchingOptions } from '@/utils/workout-matching'
...
const matchBulkSchema = z.object({
  activities: z.array(z.object({
    ...
    start_date: z.string(),
    trainer: z.boolean().optional(),
  })).min(1).max(100), // Limit to 100 activities per batch
  options: z.object({
    dateTolerance: z.number().min(0).max(7).optional().default(1),
    distanceTolerance: z.number().min(0).max(1).optional().default(0.15),
    durationTolerance: z.number().min(0).max(1).optional().default(0.2),
...
export async function POST(request: NextRequest) {
  try {
    ...
    let validatedData: { activity?: PartialStravaActivity; activities?: PartialStravaActivity[]; options?: MatchingOptions }
    ...
    if (userWorkouts.length === 0) {
      ...
      return NextResponse.json({ 
        matches: isBulk ? {} : [],
        summary: {
          total: { activities: isBulk ? validatedData.activities!.length : 1, workouts: 0, matches: 0 },
    ...
    if (isBulk) {
      // Bulk matching
      logger.debug(`Matching ${validatedData.activities!.length} activities against ${userWorkouts.length} workouts`)
      ...
      summary = {
        ...
        suggestions: matches.length > 0 ? matches[0].suggestions : ['No matches found for this activity']
      }
    ...
    logger.error('Match request failed', { error: error instanceof Error ? error.message : 'Unknown error' })
