...
export default function RunnersPageClient({ user: _user }: RunnersPageClientProps) {
  ...
  // renderRunnerCard moved inside ConnectedRunnersGrid

  const renderConnectedRunners = () => (
    <ConnectedRunnersGrid onDiscover={() => setActiveTab('discover')} />
  )
...
function ConnectedRunnersCount() {
...
function ConnectedRunnersGrid({ onDiscover }: { onDiscover: () => void }) {
  const connectedRunners = useAtomValue(connectedRunnersAtom)
