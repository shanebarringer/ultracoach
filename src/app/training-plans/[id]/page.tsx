      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 bg-white dark:bg-gray-900 rounded-lg shadow">
        <div className="mb-8">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-8">
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-6">Training Phases</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {trainingPlan.plan_phases.sort((a, b) => a.order - b.order).map(phase => (
                  <div key={phase.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                    <h3 className="font-medium text-gray-900 dark:text-white">{phase.phase_name}</h3>
                    <p className="text-sm text-gray-600 dark:text-gray-300">Duration: {phase.duration_weeks} weeks</p>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-center py-8">
                <p className="text-gray-500 dark:text-gray-400">No training phases defined for this plan.</p>
              </div>
            )}
          </div>
          ...
          {(trainingPlan.previous_plan || trainingPlan.next_plan) && (
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-8">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-6">Plan Sequence</h2>
              <div className="flex justify-between items-center">
                {trainingPlan.previous_plan ? (
                  <Link href={`/training-plans/${trainingPlan.previous_plan.id}`} className="flex items-center text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300">
                    <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                    </svg>
                ...
                {trainingPlan.next_plan ? (
                  <Link href={`/training-plans/${trainingPlan.next_plan.id}`} className="flex items-center text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">{trainingPlan.title}</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              <div className="bg-blue-50 dark:bg-blue-900 p-4 rounded-lg">
                ...
                <p className="text-blue-700 dark:text-blue-300">{formatDate(trainingPlan.created_at)}</p>
              </div>
              ...
              {trainingPlan.race && (
                <div className="bg-green-50 dark:bg-green-900 p-4 rounded-lg">
                  ...
                  <p className="text-green-700 dark:text-green-300 text-sm">{formatDate(trainingPlan.race.date)}</p>
                </div>
              )}
              ...
              {trainingPlan.goal_type && (
                <div className="bg-purple-50 dark:bg-purple-900 p-4 rounded-lg">
                  ...
                  <p className="text-purple-700 capitalize dark:text-purple-300">{trainingPlan.goal_type.replace('_', ' ')}</p>
                </div>
              )}
              ...
              {trainingPlan.plan_type && (
                <div className="bg-yellow-50 dark:bg-yellow-900 p-4 rounded-lg">
                  ...
                  <p className="text-yellow-700 capitalize dark:text-yellow-300">{trainingPlan.plan_type.replace('_', ' ')}</p>
                </div>
              )}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          {workouts.length === 0 ? (
            ...
            <div className="space-y-8">
              {trainingPlan?.plan_phases?.sort((a, b) => a.order - b.order).map(phase => (
                <div key={phase.id}>
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">{phase.phase_name}</h3>
                  {workoutsByPhase[phase.id] && workoutsByPhase[phase.id].length > 0 ? (
                            ...
                            <div className="flex-1">
                              ...
                              {workout.coach_feedback && (
                                <div className="mt-2">
                                  ...
                                  <p className="text-sm text-gray-700 dark:text-gray-200 mt-1">{workout.coach_feedback}</p>
                                </div>
                              )}
                            </div>
                    ...
                    <div className="text-gray-500 dark:text-gray-400 text-sm">No workouts planned for this phase.</div>
                  )}
                </div>
              ))}
              ...
              {ungroupedWorkouts.length > 0 && (
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Other Workouts</h3>
                  <div className="space-y-4">
