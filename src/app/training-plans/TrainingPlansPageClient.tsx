...
/**
 * Training Plans Page Client Component (Inner - Suspense-enabled)
 *
 * Handles training plans interactivity and state management.
 * Wrapped in Suspense boundary by parent component.
 * Uses async atom that throws Promise for Suspense to catch.
 */
function TrainingPlansPageClientInner({ user }: Props) {
...
/**
 * Training Plans Page Client Component (Exported - with Suspense wrapper)
 *
 * Wraps the inner component with Suspense boundary.
 * This allows the async atom to throw Promise for loading state.
 */
export default function TrainingPlansPageClient(props: Props) {
  return (
    <Suspense fallback={<TrainingPlansPageSkeleton />}>
      <TrainingPlansPageClientInner {...props} />
    </Suspense>
  )
}
