    ...
    >
      <ModalContent>
        ...
        <ModalBody className="overflow-y-auto py-6">
          ...
          />
          <Suspense
            fallback={<div className="py-8 text-center text-default-600">Loading users…</div>}
          >
            <UsersList
...
function UsersList({
  ...
  const usersPool = userType === 'coach' ? connectedRunners : availableCoaches
  const q = (searchTerm || '').toLowerCase()
  const filteredUsers = useMemo(
    () =>
      usersPool.filter((user: User) => {
    ...
    return (
      <div className="text-center py-8">
        ...
        <h3 className="mt-2 text-sm font-medium text-gray-900">
          {searchTerm
            ? 'No users found'
            : `No ${userType === 'coach' ? 'runners' : 'coaches'} found`}
        </h3>
