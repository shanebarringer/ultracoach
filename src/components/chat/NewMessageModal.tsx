...
export default function NewMessageModal({ isOpen, onClose }: NewMessageModalProps) {
  ...
  // Defer async reads to Suspense child
  ...
  // Filtering is handled in Suspense child component based on searchTerm
          ...
          <div className="space-y-4">
            <Suspense fallback={<div className="text-center py-8">Loading users…</div>}>
              <AvailableUsersSection
                role={session?.user?.userType as 'coach' | 'runner' | undefined}
...
function AvailableUsersSection({
  role,
  searchTerm,
  onSelect,
}: {
  role: 'coach' | 'runner' | undefined
  searchTerm: string
  ...
  const users = (role === 'coach' ? connectedRunners : availableCoaches) || []
    ...
    return (
      <div className="text-center py-8">
        ...
        <h3 className="mt-2 text-sm font-medium text-gray-900">
          {searchTerm ? 'No users found' : `No ${role === 'coach' ? 'runners' : 'coaches'} found`}
        </h3>
