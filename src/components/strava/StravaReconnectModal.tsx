 ...
 */
const StravaReconnectModal = memo(({ isOpen, onClose, reason = 'manual' }: StravaReconnectModalProps) => {
      ...
      <ModalContent>
        <ModalHeader className="flex flex-col gap-2">
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-lg bg-${reasonInfo.color}/10`}>
              {reasonInfo.icon}
            </div>
            <div>
            ...
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between w-full">
                ...
                <Chip
                  ...
                  variant="flat"
                  startContent={stravaState.isConnected ? <Wifi className="h-3 w-3" /> : <WifiOff className="h-3 w-3" />}
                >
              ...
              {!testing && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-center gap-2">
                    <CheckCircle2 className={`h-4 w-4 ${stravaState.connection?.connected ? 'text-success' : 'text-default-400'}`} />
                    <span className="text-sm">Basic Connection</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <CheckCircle2 className={`h-4 w-4 ${stravaState.canSync ? 'text-success' : 'text-default-400'}`} />
                    <span className="text-sm">Activity Access</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <CheckCircle2 className={`h-4 w-4 ${stravaState.activities && stravaState.activities.length > 0 ? 'text-success' : 'text-default-400'}`} />
                    <span className="text-sm">Recent Activities</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <CheckCircle2 className={`h-4 w-4 ${!stravaState.needsReconnect ? 'text-success' : 'text-default-400'}`} />
                ...
                {diagnostics.map((issue, index) => (
                  <div 
                    ...
                    className={`p-3 rounded-lg border ${
                      issue.type === 'error' ? 'bg-danger/10 border-danger/20' :
                      issue.type === 'warning' ? 'bg-warning/10 border-warning/20' :
        ...
        <ModalFooter className="gap-3">
          ...
          <Button
            ...
            startContent={reconnecting ? <Spinner size="sm" /> : <ExternalLink className="h-4 w-4" />}
          >
