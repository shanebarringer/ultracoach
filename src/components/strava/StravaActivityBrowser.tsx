...
const StravaActivityBrowser = memo(({ 
  ...
  const filteredActivities = useMemo(() => {
    ...
    // Type filter
    if (filters.type !== 'all') {
      filtered = filtered.filter((activity: StravaActivity) => activity.type.toLowerCase() === filters.type)
    }
    ...
    if (filters.syncStatus !== 'all') {
      filtered = filtered.filter((activity: StravaActivity) => {
        ...
        const hasHighConfidenceMatch = matches && matches.some((m: WorkoutMatch) => m.confidence > 0.6)
  ...
  const handleActivitySelect = useCallback((activityId: number, selected: boolean) => {
    const newSelected = new Set(selectedActivities)
    if (selected) {
  ...
  const handleBulkMatch = useCallback(async () => {
    const selectedActivityList = activities?.filter((a: StravaActivity) => selectedActivities.has(a.id)) || []
    ...
    try {
      ...
      for (let i = 0; i <= selectedActivityList.length; i++) {
        setBulkOperation(prev => ({
          ...
          currentActivity: i < selectedActivityList.length ? selectedActivityList[i].name : undefined,
        }))
      ...
      const matchedCount = Array.from(matches.values()).flat().filter(m => m.confidence > 0.5).length
  ...
  const getActivityMatchInfo = useCallback((activity: StravaActivity) => {
    const matches = activityMatches.get(activity.id)
    if (!matches || matches.length === 0) {
      ...
      <ModalContent>
        <ModalHeader className="flex flex-col gap-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-foreground">Strava Activity Browser</h2>
              <p className="text-sm text-foreground-600">
                Browse, match, and sync your Strava activities with planned workouts
              </p>
            </div>
          </div>
              ...
              <CardBody className="py-3">
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  <div className="text-center">
                    <div className="text-lg font-semibold text-foreground">{matchingSummary.total.activities}</div>
                    <div className="text-xs text-foreground-600">Activities</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-semibold text-success">{matchingSummary.by_confidence.exact}</div>
                    <div className="text-xs text-foreground-600">Perfect Matches</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-semibold text-primary">{matchingSummary.by_confidence.probable}</div>
                    <div className="text-xs text-foreground-600">Good Matches</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-semibold text-warning">{matchingSummary.by_confidence.possible}</div>
                    <div className="text-xs text-foreground-600">Possible</div>
                  </div>
                  <div className="text-center">
                    <div className="text-lg font-semibold text-danger">{matchingSummary.by_confidence.conflicts}</div>
                    <div className="text-xs text-foreground-600">Conflicts</div>
                  </div>
                </div>
              </CardBody>
            </Card>
            ...
            <CardBody className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <Checkbox
                    isSelected={selectedActivities.size === paginatedActivities.length && paginatedActivities.length > 0}
                    isIndeterminate={selectedActivities.size > 0 && selectedActivities.size < paginatedActivities.length}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">
                        {bulkOperation.operation === 'match' ? 'Matching Activities...' : 'Syncing Activities...'}
                      </span>
                    </div>
                      ...
                      <TableRow key={activity.id}>
                        <TableCell>
                          <Checkbox
                            ...
                            onValueChange={(selected) => handleActivitySelect(activity.id, selected)}
                          />
                        </TableCell>
                        ...
                        <TableCell>
                          {matchInfo.hasMatch ? (
                            <Tooltip content={`${(matchInfo.confidence * 100).toFixed(0)}% confidence`}>
                              <Chip
                                size="sm"
                                variant="flat"
                                color={
                                  matchInfo.confidence > 0.8 ? 'success' :
                                  matchInfo.confidence > 0.6 ? 'primary' :
                                  matchInfo.confidence > 0.3 ? 'warning' : 'danger'
                                }
                                startContent={
                                  matchInfo.matchType === 'exact' ? <CheckCircle2 className="h-3 w-3" /> :
                                  matchInfo.matchType === 'conflict' ? <AlertTriangle className="h-3 w-3" /> :
